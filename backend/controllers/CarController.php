...
class CarController extends BController {
    ...
    public function actionShop_list() {
        ...
        if (Yii::$app->request->isAjax) {
            ...
            if($_REQUEST["shopname"]){
                $where.=" and {{%car_shop}}.shop_name like '%".trim($_REQUEST["shopname"])."%'";
            }
            if(!empty($_REQUEST['area']))
            {
                $where.=' and {{%car_shop}}.area='.$_REQUEST['area'];
            }else if(!empty($_REQUEST['city']))
            {
                $where.=' and {{%car_shop}}.city='.$_REQUEST['city'];
            }else if(!empty($_REQUEST['province']))
            {
                $where.=' and {{%car_shop}}.province='.$_REQUEST['province'];
            }


            $cot = $shopModel->select("count({{%car_shop}}.id) as cot",$where)
                ->join('LEFT JOIN','{{%fans}}','{{%fans}}.id = {{%car_shop}}.uid')
                ->one();


            $shoplist = $shopModel->select('{{%car_shop}}.*,{{%fans}}.nickname',$where)
                ->join('LEFT JOIN','{{%fans}}','{{%fans}}.id = {{%car_shop}}.uid')
                ->page($page,$pagesize)
                ->orderBy('{{%car_shop}}.id desc')
                ->all();
            $satus=[0=>'冻结',1=>'已申请',2=>'申请通过',3=>'申请被拒绝'];
            ...
            $res = ['IsOk' => 1,'total'=> $cot['cot'],'rows'=>$shoplist];
            return json_encode($res);
        }
        $area=new Area();
        $province=$area->getProvince();

        return      $this->render ( 'shop_list' ,array('cur'=>1,'province'=>$province));
    ...
    public function actionShop_edit() {
        ...
        if (Yii::$app->request->post()) {
            ...
            unset($data['id']);
            unset($data['file']);
            (new Car_shop())->upData($data,$where);
            $this->redirect('shop_list.html');
        }
        $where = " {{%car_shop}}.id={$id} ";
        $shopModel = new Car_shop();
        $shopinfo = $shopModel->select("{{%car_shop}}.*,{{%fans}}.nickname",$where)
            ->join('LEFT JOIN','{{%fans}}','{{%fans}}.id = {{%car_shop}}.uid')
            ->one();
        $area=new Area();
        $province=$area->getProvince();
        $address=(new Area())->getData('`name`,`code`', 'all', 'code in ("'.$shopinfo['province'].'","'.$shopinfo['city'].'","'.$shopinfo['area'].'")');
        ...
        return $this->render('shop_edit',['data'=>$shopinfo ,'address'=>$address,'province'=>$province]);
...
//            }
    ...
    protected function setWithdrawalsWhere(){
        ...
        $where=' {{%car_withdrawals}}.id<>0 ';
        if(!empty($payee)){
            $where.=' AND  {{%car_withdrawals}}.payee ="'.$payee.'"';
        }

        if(!empty($account)){
            $where.=' AND  {{%car_withdrawals}}.account ="'.$account.'"';
        }

        if(!empty($shop_name)){
            $where.=' AND  {{%car_shop}}.shop_name ="'.$shop_name.'"';
        }

        if(!empty($status)){
            $where.=' AND  {{%car_withdrawals}}.status ="'.$status.'"';
        }
        return $where;
    }
    public function actionWithdrawals_log() {
        ...
        $wstatus=Car_withdrawals::$status;
        if (Yii::$app->request->isAjax) {
            $page = $request->get("pageNumber",1);
            $pagesize = $request->get("pageSize",10);
            $model = new Car_withdrawals();
            $where = self::setWithdrawalsWhere();
            $cot = $model->select("count({{%car_withdrawals}}.id) as cot",$where)
                ->join('LEFT JOIN','{{%car_shop}}','{{%car_withdrawals}}.shop_id = {{%car_shop}}.id')
                ->one();

            $list = $model->select('{{%car_withdrawals}}.*,{{%car_shop}}.shop_name',$where)
                ->join('LEFT JOIN','{{%car_shop}}','{{%car_withdrawals}}.shop_id = {{%car_shop}}.id')
                ->page($page,$pagesize)
                ->orderBy('{{%car_withdrawals}}.id desc')
                ->all();

            $satus=[0=>'删除',1=>'已申请',2=>'已打款',3=>'已拒绝'];
            foreach ($list as $key=>$val){
                $list[$key]['status'] =$satus[$val['status']] ;
    ...
    public function actionW_down(){
        ...
        $list = (new Car_withdrawals())->select('{{%car_withdrawals}}.*,{{%car_shop}}.shop_name',$where)
            ->join('LEFT JOIN','{{%car_shop}}','{{%car_withdrawals}}.shop_id = {{%car_shop}}.id')
            ->page($page,$pagesize)
            ->orderBy('{{%car_withdrawals}}.id desc')
            ->all();
    ...
    public function actionWithdrawals_edit() {
        ...
        $status=[0=>'已删除',1=>'已申请',2=>'已打款',3=>'已拒绝'];
        $where = " {{%car_withdrawals}}.status != 0  and {{%car_withdrawals}}.id = $id";
        $info = $model->select("{{%car_withdrawals}}.* , {{%car_shop}}.shop_name, {{%fans}}.nickname",$where)
            ->join('LEFT JOIN','{{%car_shop}}','{{%car_withdrawals}}.shop_id = {{%car_shop}}.id')
            ->join('LEFT JOIN','{{%fans}}','{{%car_withdrawals}}.uid = {{%fans}}.id')
            ->one();

        $info['apply_time'] = !empty($info['apply_time'])?date("Y-m-d H:i:s",$info['apply_time']):'--';//playmoney_time
    ...
    public function actionPlay_money(){
        ...
        if (Yii::$app->request->isAjax) {
            ...
            //支付宝转账
            $account = (new Car_payee())->select('*',['id'=>$info['payee_id'],'type'=>'1'])->one();
            if($account){
            ...
            try{
                ...
                $shopModel->upData(array('amount'=>'amount-'.$info['amount'],'already_amount'=>'already_amount+'.$info['amount']),'`id` ='.$info['shop_id']);
                $trans->commit();
                $this->write_log($log);
            }catch (\Exception $e){
                $trans->rollBack();
...
//兑换记录
    ...
    public function actionAccount_edit(){
        ...
        if (Yii::$app->request->isAjax) {
            ...
            $id=$data['payee_id'];
            unset($data['payee_id']);
            if(! $model->upData($data ,'id='.intval($id))) return $this->json(0);
            return $this->json(1);
        }

        $where = "{{%car_payee}}.id=".intval($request->get('id'));
        $info = $model->select("{{%car_payee}}.*,{{%car_shop}}.shop_name,{{%fans}}.nickname",$where)
            ->join('LEFT JOIN','{{%car_shop}}','{{%car_payee}}.shop_id = {{%car_shop}}.id')
            ->join('LEFT JOIN','{{%fans}}','{{%car_payee}}.uid = {{%fans}}.id')
            ->one();
        $info['c_time']=date("Y-m-d H:i:s",$info['c_time']);
        return      $this->render ( 'account_edit',['data'=>$info] );
    }
    //下载导入模板
    public function actionDexcel(){
        $rootpath = Yii::$app->params['rootPath'];
        return Yii::$app->response->sendFile($rootpath.'./static/tpl/car_card.xls');
    }
}