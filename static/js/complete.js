	...
	var bigAutocomplete = new function(){
		...
		this.init = function(){
			...
			$("#bigAutocompleteContent").delegate("tr", "click", function() {
				bigAutocomplete.currentInputText.val( removeHTMLTag($(this).find("div:last").html()));
		...
		this.autocomplete = function(param){
			...
			$this.on('keyup input paste',function(event) {
				...
				if(!isFunctionalKey && (ctrl || (!ctrl && !!k) || (!ctrl && !!k)) || !k ){
					...
					$bigAutocompleteContent.css({"top":offset.top + h - document.body.scrollTop,"left":offset.left - document.body.scrollLeft});
...
//				}
			...
			$this.focus(function(){
				...
				$bigAutocompleteContent.width($this.outerWidth() - 2);
				$bigAutocompleteContent.css({"top":offset.top - document.body.scrollTop + h,"left":offset.left - document.body.scrollLeft,"display" : "block"})
				if(data != null && $.isArray(data) ){
					var data_ = new Array();
					for(var i=0;i<data.length;i++){
						if(data[i].title.indexOf(keyword_) > -1){
