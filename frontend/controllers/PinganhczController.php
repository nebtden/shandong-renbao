...
class PinganhczController extends PController
{
    /**
     * 返回盛大错误代码
     * 200 请求成功
    ...
    protected function couponDetail($code,$storeName)
    {
        ...
        try {
            ...
            foreach ($list as $key=>$val){
                //从平安查询卡券详情
                $res = $pingAn->couponDetail($code,$val['outlet_id'],$val['third_outlet_no']);
            ...
            $insertData = [
                ...
                'merchant_shop' => $shopNo,//$resData['productItemList'][0]['outletList'][0]['outletNo'],
                //'total_amount' => $resData['productItemList'][0]['totalAmount'],
            ];


            $this->log('insertdata',json_encode($insertData));
        } catch (\Exception $e){
