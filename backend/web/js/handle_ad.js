...
function actionFormatter(value, row, index) {
    var pus, next = '';
    var check = '<a class="check" href="javascript:void(0)" title="广告内容"><i class="glyphicon glyphicon-eye-open"></i>广告内容</a>&nbsp;&nbsp;';
    if (more) {
...
window.actionEvents = {
    ...
    'click .edit': function (e, value, row, index) {
        ...
        if (imgcon) {
            window.location.href = eurl + '?id=' + row.id;//+ '&type='.row.type
        }
    ...
    'click .check': function (e, value, row, index) {
        console.log(row);
        window.location.href = checkurl + '?nid=' + row.id + '&type=' + row.typeid;
    }
    ,
