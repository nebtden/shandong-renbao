...
$('#sousuo').click(function () {
    ...
    var opt2 = $('#sec').val();
    var opt3 = $('#aid').val();
    $('.table').bootstrapTable('refresh', {query: {uname: opt1, id: opt2, aid: opt3}}, function (row) {
        console.log(row);
    });
});
...
function actionFormatter(value, row, index) {
    ...
    var aArr = [];
    if (more) {
        next = '<a class="like" href="javascript:void(0)" title="规格数据"><i class="glyphicon glyphicon-plus-sign"></i>规格数据</a>&nbsp;&nbsp;';
    }
    ...
    if (next) aArr.push(next);
    if (pus && eurl) aArr.push(pus);
    if (typeof hrefArr !== 'undefined' && hrefArr) {
        for (var i = 0; i < hrefArr.length; i++) {
            aArr.push('<a class="ml10" href="' + hrefArr[i].href + '?' + hrefArr[i].field + '=' + row[hrefArr[i].field] + '" title="' + hrefArr[i].title + '"><i class="glyphicon glyphicon-plus-sign"></i> ' + hrefArr[i].title + '</a>&nbsp;&nbsp;')
        }
    }
    if (durl) remove = '<a class="remove ml10" href="javascript:void(0)" title="Remove"><i class="glyphicon glyphicon-remove"></i>删除</a>';
    if (remove) aArr.push(remove);
    return aArr.join('');
}
