...
class CarrescueController extends CloudcarController
{
    ...
    public function actionIndex()
    {
        ...
        $rescueItems = CarCoupon::$coupon_faulttype_web;
    ...
    public function actionRescuecp()
    {
        ...
        foreach ($temp as $cp) {
            //选取相关场景的券，再判断是否过期
            if ((int)$cp['use_scene'] === (int)$scene || $cp['use_scene'] == 5) {
    ...
    public function actionPaidan()
    {
        ...
        try {
            ...
            $usedCoupon = $couponObj->useCoupon($coupon_id);
            if ($usedCoupon === false) {
            ...
            $info = [
                ...
                'm_id' => $parentOrder['id'],
                'company' => (int)$usedCoupon['company'],
            ];
            $rs = $this->send_rescue_order($info, $usedCoupon);
            if (!$rs) {
        ...
        $trans->commit();
        return $this->json(1, 'ok', ['m_id'=>$parentOrder['id'],'id' => $detail['id'], 'orderno' => $detail['orderid']]);
    }

    protected function send_rescue_order($info, $coupon)
    {
        $type = (int)$coupon['company'];
        //type = 1使用中联的接口
        //type = 2使用九天的接口
        //当前中联
        $data = [
        ...
        if($type === 1){
            ...
            $data['cardNo'] = $coupon['coupon_sn'];
            $data['cardPassword'] = $coupon['coupon_pwd'];
            $res = W::rescuejiutianapi($data);
            if((int)$res['error_code'] === 0) return true;
        }
        return false;
    }
