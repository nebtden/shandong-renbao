...
<a href="#" onclick="canyu()" class="lj-btn"><img id='choujiang' src="images/lj-btn.png" style="width: 3.12rem" alt=""></a>
    ...
    <script>
        ...
        function canyu(){
            <?php if($isLogin): ?>
                if(isFirst) {
                    sharePage();
                    $('#choujiang').attr('src','images/choujiang.png');
                    isFirst = false;
                    console.log(isFirst);
                }else {
                    <?php if(time() < $activeDate['start_time']):?>
                    YDUI.dialog.toast('活动尚未开始', 'error', 1000, function () {
                    });
                    <?php elseif (time() > $activeDate['end_time']):?>
                    YDUI.dialog.toast('活动已结束', 'error', 1000, function () {
                    });
                    <?php else:?>
