...
//                die;
    ...
    protected function show_4() {
        ...
        $begin = ($begin >= 1) ? $begin : 1;
//        $return = '<div class="am-cf">共 ' . $this->total_rows . ' 条记录 <div class="am-fr"><ul class="am-pagination">';
        $return = '<div class="pageList"><ul class="clearfix">';
        $return .= $this->first_page('首页');
        $return .= $this->up_page('<<');
//        var_dump($this->now_page);die;
        for ($i = $begin; $i <= $begin + $plus * 2; $i ++) {
            if ($i > $this->total_pages) {
                break;
            }
            if ($i == $this->now_page) {
                $return .= "<li class='cur'><a>$i</a></li>\n";
            } else {
                $return .= $this->_get_link($i, $i) . "\n";
            }
        }
